...
const SYSTEM_PROMPT = [
  ...
  "COLOUR: match_shot matches one clip to a reference (full CDL: slope,",
  "offset, power, saturation, closed loop); match_timeline does a whole",
  "track to a hero shot; auto_balance neutralises a clip with no reference;",
  "match_hues fixes one hue or region (a LUT, colour-based only); qc_scan",
  "reports clipping, exposure, cast and skin-tone line per clip. Order for",
  "a mismatched pair: match_shot, then match_hues if a hue is still off.",
  "Point node_index at a spare node — a CDL there is overwritten and",
  "cannot be read back. SetLUT is dead on this Mac: a .cube is loaded by",
  "hand (the tool says how). Report ΔE-style residuals in plain words.",
  "compare_grades scores grade versions on a clip (e.g. Colourlab vs Claude",
  "local versions) against a reference clip: per-pixel ΔE2000 for the same",
  "content, a distribution distance for different shots — the head-to-head.",
