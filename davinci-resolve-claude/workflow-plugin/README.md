...
your style profile, and `/style` reads back what it has learned (the
profile is `~/ClaudeAssistantStyle/car-edits.json`, shared with the After
Effects panel, which has the same `/train`). `/help`, `/new`, `/history`
and `/copy` are handled by the panel itself. An unknown `/word` is answered by the panel ("No command
